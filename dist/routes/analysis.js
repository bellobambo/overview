...
router.post('/:submissionId', async (req, res) => {
    try {
        ...
        if (events.length === 0) {
            return (0, apiResponse_1.sendError)(res, 400, 'No keystroke data found for analysis.');
        }
        ...
        let revisionScore = 0;
        if (revisionRatio >= 0.05 && revisionRatio <= 0.30)
            revisionScore = 100;
        else if (revisionRatio < 0.05)
            revisionScore = (revisionRatio / 0.05) * 100;
        else
            revisionScore = 50; // Too much deletion is weird but not necessarily AI
        ...
        if (wpms.length > 5) {
            ...
            if (stdDev < 5 && avgWpm > 50)
                varianceScore = 20; // Robotic
            else
                varianceScore = Math.min(100, (stdDev / 15) * 100);
        }
        let pasteScore = 100;
        ...
        if (pasteRatio > 0)
            pasteScore = Math.max(0, 100 - (pasteRatio * 200));
        let tabScore = 100;
        ...
        const overallScore = Math.round((revisionScore * 0.25) +
            (varianceScore * 0.25) +
            (pauseScore * 0.20) +
            (pasteScore * 0.15) +
            (tabScore * 0.15));
        ...
        const result = { behavioralScore, sessionStats };
...
${JSON.stringify(payload, null, 2)}`;
                try {
                    ...
                    if (aiJson && Array.isArray(aiJson.segment_analyses)) {
                        ...
                        result.segments = aiJson.segment_analyses.map((sa) => {
